...
from openicl.icl_inferencer import LlamaInferencer
...
def post_process(task_name, all_predictions) :
    def _format(response) :
        if "\n\n" in response and len(response.split("\n\n")) > 1 :
            response = " ".join(response.split("\n\n")[1:])  # remove the greetings, e.g., "Sure, I can do that!\n\n xxx"
        ...
        if task_name in ['xcopa', 'xstorycloze'] :
            ...
            if match :
                response = match.group(0)
            if ":" in response:  # avoid code switching, e.g. eu "Aukera zenbakia: 1"  ru "Выбор номер: 1"
                response = "Choice number: " + response.split(":")[1].strip()[0]
...
def compuate_metrics(task_name, all_predictions, all_references) :
    if task_name in ['xquad', 'tydiqa'] :
        ...
        evaluator = EMEvaluator()  # exact match for classification tasks

    # post-processing the response before automatic evaluation
    all_predictions = post_process(task_name, all_predictions)
...
def run_task(model, model_args, data_args, training_args, icl_args) :
    ...
    for lang, lang_name in zip(languages, language_names) :
        ...
        inferencer = LlamaInferencer(model,
                                     tokenizer_name=model_args.tokenizer_name,
                                     max_model_token_num=model_args.max_seq_len,
                                     generation_kwargs={"max_new_tokens" : max_new_tokens},  # greedy decoding
                                     model_parallel=True,
