...
class TopkRetriever(BaseRetriever):
    ...
    def __init__(self,
        ...
        #TODO: why use tokenizer here?? -> raised an issue on github
    ...
    def knn_search(self, ice_num):
        ...
        for entry in tqdm.tqdm(res_list, disable=not self.is_main_process):
            ...
            # Added:  deal with index_ds == test_ds  (for qa datasets)
            near_ids = self.index.search(embed, ice_num+1)[1][0].tolist()

            if self.index_split != self.test_split:
                near_ids = near_ids[:self.ice_num]
            else:
                near_ids = [n for n in near_ids if n != idx][:self.ice_num]
    ...
    def forward(self, dataloader, process_bar=False, information=''):
        ...
        for _, entry in enumerate(_dataloader):
            with torch.no_grad():
                ...
                res = self.model.encode(raw_text, show_progress_bar=False, normalize_embeddings=True) # in order to use cosine similarity in faiss
