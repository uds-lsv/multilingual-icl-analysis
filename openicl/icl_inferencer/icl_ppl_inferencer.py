        ...
        for label in labels :
            ...
            for idx in range(len(ice_idx_list)) :
                ...
                if self.max_model_token_num is not None and self.api_name != 'gpt3' :
                    ...
                    while len(ice_idx_list[idx]) > 0 and prompt_token_num > self.max_model_token_num :  # if the input is too long -> reduce ice
            ...
            for idx in trange(0, len(prompt_list), self.batch_size, disable=not self.is_main_process) :
                ...
                with torch.no_grad() :
                    if normalizing_str is not None:
                        if len(normalizing_str)>0:  # method 1: conditional normalization
