...
class Seq2seq_PPLInferencer(BaseInferencer) :
    ...
    def inference(self, retriever: BaseRetriever, ice_template: Optional[PromptTemplate] = None,
                  ...
                  output_json_filename: Optional[str] = None, normalizing_str: Optional[str] = '') -> List :
        ...
        for label in labels :
            ...
            for idx in range(len(ice_idx_list)) :
                ...
                if self.max_model_token_num is not None and self.api_name != 'gpt3' :
                    ...
                    while len(ice_idx_list[
                                  idx]) > 0 and prompt_token_num > self.max_model_token_num :  # if the input is too long -> reduce ice
                ...
                if normalizing_str is not None :
                    ...
                    context = prompt_sep[0 :sep_pos]       # input_texts
                    answer = prompt_sep[sep_pos :].replace(sep_token, '')  # answers
            ...
            for idx in trange(0, len(prompt_list), self.batch_size, disable=not self.is_main_process) :
                ...
                if normalizing_str is not None :
                    sub_answer_list = normalizing_prompt_list[idx :idx + self.batch_size]

                with torch.no_grad() :
                    if normalizing_str is not None :
                        sub_res = self.__get_ppl(input_texts=sub_prompt_list, answers=sub_answer_list)
                    else :
                        raise ValueError
                for res, prompt in zip(sub_res, sub_prompt_list) :
                    sub_ppl_list.append(res)
                    output_handler.save_prompt_and_ppl(label, prompt[len(ice[index]) :], prompt, res, index)
                    index = index + 1
    ...
    def __get_ppl(self, input_texts: List[str], answers: List[str]) :
        self.tokenizer.padding_side = "right"
        inputs = self.tokenizer(input_texts, padding=True, return_tensors='pt', truncation=True,
                                max_length=self.max_model_token_num)
        inputs['labels'] = self.tokenizer(answers, padding=True, return_tensors='pt')['input_ids']
