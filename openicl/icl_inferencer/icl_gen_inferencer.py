    ...
    def inference(self, retriever: BaseRetriever, ice_template: Optional[PromptTemplate] = None,
        ...
        for entry in tqdm(dataloader, disable=not self.is_main_process):
            ...
            if not self.call_api:
                with torch.no_grad():
                    tokenized_data = self.tokenizer.batch_encode_plus(entry, padding=True, return_tensors='pt',
                                    truncation=True, max_length=self.max_model_token_num).to(self.device)
                    prompt_len = int(tokenized_data.attention_mask.shape[1])
                    if ('t5' in self.tokenizer_name) or ('t0' in self.tokenizer_name):
                        prompt_len = 0
